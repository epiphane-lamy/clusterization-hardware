...
- With the **macro-backed wrapper memories** (see [ADR-0007](decisions/0007-memory-macro-wrappers.md) and the `docs/blocks/*_mem_wrapper.md` files) — i.e. the same testbenches, but exercising the wrapper RTL that instantiates behavioral models of the ASIC black-box macros, run via a dedicated `sim_rtl_bb_v2` Makefile target for the v2 architecture and `sim_rtl_bb` for the v1 ("bb" for black box). These targets load a different filelist, pointing at the wrapper memories located under `frontend/synth_files` (the same source tree used for synthesis) instead of the plain behavioral memories.
